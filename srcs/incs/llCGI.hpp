...
class llCGI {
 public:
	llCGI(const HttpRequest &request, const std::string &req_path, const RequestLocation *request_location);
	virtual ~llCGI(void);
 ...
 private:
	...
	std::string GetExecutable_(const std::string &extension, const RequestLocation *request_location);
